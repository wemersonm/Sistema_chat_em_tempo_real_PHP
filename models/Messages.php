 <?php 
	...
	class Messages extends Model{
		...
		public function insertMessage($uid,$id_group,$msg,$msg_type='text'){
			$stmt = $this->db->prepare('INSERT INTO chat.messages(id_user,id_group,date_msg,msg,msg_type)
				VALUES(:id_user,:id_group,NOW(),:msg,:msg_type)');
			$stmt->bindValue(":id_user",$uid);
			$stmt->bindValue(":id_group",$id_group);
			$stmt->bindValue(":id_user",$uid);
			$stmt->bindValue(":msg",$msg);
			$stmt->bindValue(":msg_type",$msg_type);
			if($stmt->execute()){
				return true;
			}
			return false;
